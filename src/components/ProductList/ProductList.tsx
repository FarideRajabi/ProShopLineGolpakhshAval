import { useState, useEffect, FC } from "react";
import axios from 'axios'
import AuthService from './../../services/auth.services'
import OrderForm from "../OrderForm/OrderForm";
import { ProductListPropsType, ProductsType } from "../../types/components/ProductList";
import { Col, InputNumber, Table, Input, Row, Image } from 'antd';
import './ProductList.css'
import { CodeSandboxOutlined } from "@ant-design/icons";
const { Search } = Input

const ProductList: FC<ProductListPropsType> = ({ id, type }) => {
    const user = JSON.parse(localStorage.getItem("user")!);
    const [char, setChar] = useState("")
    const [loading, setLoading] = useState(false)

    let counter = 0
    const [products, setProducts] = useState<ProductsType[]>([{
        id: 0,
        imageLocation: "",
        productPrice: 0,
        productTitle: "",
        quantity: 0,
        quantityPerPackage: 0,
        requestedQuntity: 0,
        key: 0,
    }])

    const handleChangeQuntity = (text: number, value: null | number) => {

        const index = products.findIndex((item) => item.id === text);

        const newProducts = [...products];
        newProducts[index] = {
            ...newProducts[index], requestedQuntity: Number(value)
        };
        setProducts(newProducts);
    };

    const tableHead = [{
        title: <Row justify="center" >ردیف</Row>,
        width: 50,
        dataIndex: 'index',
        key: 'index',
        render: function renderCounter() {
            counter += 1
            return <Row justify="center" >{counter}</Row>;
        },
    },
    {
        title: <Row justify="center" >تصویر</Row>,
        width: 100,
        dataIndex: 'imageLocation',
        key: 'imageLocation',
        render: (path: string) => (
            <Row justify="center">
                {path ? <img
                    src={path}
                    width="50px"
                /> : <><Image
                    width="80px"
                    src="error"
                    fallback="data:image/png;base64," /><></></>}
            </Row>
        ),
    },
    {
        title: <Row justify="center" >نام کالا</Row>,
        width: 200,
        dataIndex: 'productTitle',
        key: 'productTitle',
        render: (productTitle: string) => (
            <Row justify="center">
                {productTitle}
            </Row>
        ),
    },
    {
        title: <Row justify="center" >تعداد در بسته‌بندی</Row>,
        width: 100,
        dataIndex: 'quantityPerPackage',
        key: 'quantityPerPackage',
        render: (value: number) => <Row justify="center">
            <CodeSandboxOutlined style={{ fontSize: "30px" }} />
            <h3 className="badge">{value.toLocaleString()}</h3>
        </Row>,
    },
    {
        title: <Row justify="center" >قیمت مصرف کننده (ریال)</Row>,
        width: 100,
        dataIndex: 'productPrice',
        key: 'productPriceRial',
        render: (value: number) => <Row justify="center">{value.toLocaleString()}</Row>,
    },
    {
        title: <Row justify="center" >سقف سفارش</Row>,
        width: 100,
        dataIndex: '50',
        key: '50',
        render: (id: number) => <Row justify="center">50</Row>
    },
    {
        title: <Row justify="center" >تعداد سفارش</Row>,
        width: 100,
        dataIndex: 'id',
        key: 'id',
        render: (id: number) => (
            <Row justify="center">
                <InputNumber
                    value={products.find((p) => p.id === id)?.requestedQuntity}
                    onChange={(value: number | null) => handleChangeQuntity(id, value)}
                    min={0}
                    max={50}
                    maxLength={4}
                />
            </Row>
        ),
    },
    ];

    useEffect(() => {
        setLoading(true)
        if (char) {
            const options = {
                method: 'GET',
                url: `${type === "برند" ? AuthService.GetProductByBrandId : (type === "دسته بندی") ? AuthService.GetProductByCategoryId : AuthService.GetProductBySupplierId}${id}&pageNo=1&pageSize=1000&SearchText=${char}&BranchId=${ user.branchList[0].id}`,
                headers: {
                    authorization: `Bearer ${user.token}`
                }
            }
            axios.request(options).then(function (response) {
                setProducts(response.data.data.map((product: ProductsType) => {
                    return ({ ...product, requestedQuntity: 0 })
                }))
                setLoading(false)

            }).catch(function (error) {
                console.error(error)
                setLoading(false)
            })
        } else {
            const options = {
                method: 'GET',
                url: `${type === "برند" ? AuthService.GetProductByBrandId : (type === "دسته بندی") ? AuthService.GetProductByCategoryId : AuthService.GetProductBySupplierId}${id}&pageNo=1&pageSize=1000&BranchId=${ user.branchList[0].id}`,
                headers: {
                    authorization: `Bearer ${user.token}`
                }
            }
            axios.request(options).then(function (response) {
                setProducts(response.data.data.map((product: ProductsType) => {
                    return ({ ...product, requestedQuntity: 0 })
                }))
                setLoading(false)

            }).catch(function (error) {
                console.error(error)
                setLoading(false)
            })
        }
    }, [char])

    const findProductQuantity = products.filter((item: any) => item.requestedQuntity > 0)

    return (<>
        <Col xs={24} sm={12} md={12}>
            <Search
                loading={loading}
                placeholder={`جستجو ${type}`}
                onChange={(event) => {
                    const target = event.target as HTMLInputElement;
                    setChar(target.value);
                }}
            />
        </Col>

        {products.length !== 0 ? <>
            <OrderForm findProductQuantity={findProductQuantity} products={products} setProducts={setProducts} />
            <Table size="large" columns={tableHead} dataSource={products} scroll={{ x: 1500 }} />
        </>
            :
            <div>
                <div>محصولی وجود ندارد</div>
            </div>}
    </>)
}

export default ProductList
